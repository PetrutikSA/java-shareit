...
public class GetAllOwnersPastBookings extends GetAllBookingsHandler {
    @Override
    public List<Booking> handle(Long userId, BookingState state, BookingRepository bookingRepository) {
        if (state == BookingState.CURRENT)
            return bookingRepository.findAllByItemOwnerIdAndEndLessThan(userId, LocalDateTime.now());
